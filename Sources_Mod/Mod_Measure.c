...
void Mod_MeasurePoll(Measure_t* pst_Meas)
{
    ...
    case e_MeasureCal:
        ...
        {
        	/* 总点数 大于无效点数 */
        	INT16U num = pst_Meas->st_SampleNO.ul_Len - pst_Meas->uin_InvalidDots;		//计算去掉N1个无效点后剩余的样品点数

        	if (num  > pst_Meas->uin_ActiveDots)										//判断剩余点数是否大于N2个有效点数
        		num  = pst_Meas->uin_ActiveDots;

            for( i = 0; i <	num; i++)
            	f += pst_Meas->st_SampleNO.af_Buff[pst_Meas->uin_InvalidDots + i];	    //求和
            pst_Meas->lf_NO = f / num;								                    //算平均
        }
        ...
        {
        	/* 总点数 大于无效点数 */
        	INT16U num = pst_Meas->st_SampleHC.ul_Len - pst_Meas->uin_InvalidDots;		//计算去掉N1个无效点后剩余的样品点数
...
*/
    ...
    case e_MeasureStaticSample:
        MEASURE_DBG(">>MEASURE DBG:   开始静态采样\r\n");
